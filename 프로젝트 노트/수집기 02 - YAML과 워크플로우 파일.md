...
| --- | --- |
| `on:` | **언제** 실행할지 (조건·트리거) |
| `jobs:` | **무엇을** 실행할지 (실제 할 일) |

알람 시계에 비유하면 `on:`은 알람을 몇 시에 맞출지, `jobs:`는 알람이 울리면 실제로 무엇을 할지에 해당한다.

`jobs` 안의 내용은 화면에 표시되는 것이 아니라 리눅스 가상머신에서 실제로 돌아가는 명령이다. 지금은 그 명령이 `echo`라서 로그에 글자가 찍힐 뿐이고, 나중에 이 자리는 `python collector/main.py`처럼 실제 수집을 수행하는 명령으로 바뀐다.
...
jobs:                          # 할 일 목록 (여러 개 가능)
  hello:                       # 일감 하나의 이름
    runs-on: ubuntu-latest     # 어떤 컴퓨터를 빌릴지
    steps:                     # 순서대로 실행할 단계들
      - name: Say hello        # 이 단계의 라벨 (로그에 표시됨)
        run: echo "..."        # 실제 실행할 명령
```

| 키 | 의미 |
| --- | --- |
| `jobs` | 일감 목록. 여러 개 쓰면 기본적으로 동시에 실행된다 |
| `runs-on` | 실행할 가상머신 종류 (`ubuntu-latest` / `windows-latest` / `macos-latest`) |
| `steps` | 그 일감 안에서 위에서 아래로 순서대로 실행되는 단계들 |
| `name` | 로그에 표시될 이름 |
| `run` | 셸에서 실행할 명령 |

이름 중 어떤 것이 내가 지은 것이고 어떤 것이 규격인지 구분해 두면 헷갈리지 않는다.

- `hello`, `Say hello` → 내가 마음대로 지은 이름. `collect-jobs` 같은 것으로 바꿔도 된다
- `jobs`, `runs-on`, `steps`, `run`, `name` → GitHub이 정한 규격. 철자가 틀리면 인식되지 않는다

### 1-6. YAML 문법과 키 이름은 다른 층이다
...
- `"0 0 * * *"` → 또 다른 층인 cron 표현식. YAML 입장에서는 그냥 문자열이다

YAML 문법을 다 안다고 워크플로우를 쓸 수 있는 것이 아니고, 반대로 Actions 키를 외웠다고 YAML을 아는 것도 아니다. Docker Compose도 YAML을 쓰지만 키 이름은 완전히 다르다.

### 1-7. `on`은 표준 YAML에서 `true`로 읽힌다

만든 파일을 표준 YAML 파서로 돌려보니 `on:` 키가 이렇게 나왔다.

```json
{ "name": "hello-cron", "true": { "schedule": [ ... ] } }
```

YAML 1.1 명세가 `on`/`off`/`yes`/`no`를 불리언으로 취급하기 때문에 `on`이 참(true)으로 해석된 것이다. 그런데도 Actions는 정상 동작한다. GitHub이 이 문제를 알고 자기 파서에서 예외 처리를 해두었기 때문이다.

키 이름이 YAML 문법과 다른 층이라는 사실의 가장 극단적인 사례다. 다른 YAML 도구에서는 이 충돌을 피하려고 `"on"`처럼 따옴표를 씌우기도 한다.
...
- `uses` — 남이 만들어 둔 액션을 가져다 쓰는 방법 (`actions/checkout` 등)
- `needs` — 여러 job의 실행 순서를 정하는 방법
- YAML 앵커(`&`, `*`) — 중복되는 설정 묶기
- Docker Compose의 YAML — 같은 문법에 다른 키가 어떻게 쓰이는지

## 관련 노트

[[수집기 프로젝트 MOC]] · [[수집기 03 - Actions 실행과 트리거 설계]] · [[수집기 01 - 리포를 Private으로 정한 이유]]
