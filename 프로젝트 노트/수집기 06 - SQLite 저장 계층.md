...
파일에 줄을 쌓던 것을 DB에 행을 쌓는 것으로 바꾸는 단계다. `collector/store.py`를 함수 하나씩 만들어가며 기록한다. 첫 함수까지가 이 노트이고, 나머지 두 함수는 [[수집기 07 - store.py 세기와 넣기]]에 있다.
...
`mkdir(exist_ok=True)`와 완전히 같은 발상이고 이것이 **멱등성**이다. 무인 시스템에서는 이 패턴이 계속 반복된다. **이미 되어 있으면 아무것도 하지 않는다.** #멱등성 → [[개념 - 멱등성]]
...
[[수집기 프로젝트 MOC]] · [[수집기 05 - main.py 코드 분석]] · [[수집기 07 - store.py 세기와 넣기]] · [[수집기 06-1 - SQLite에 서버가 없다는 뜻]]
