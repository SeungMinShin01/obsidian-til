...
> ③ 커넥션 풀에 `timezone`과 `dateStrings` 설정이 없다. 예약 쪽에서 날짜가 UTC로 틀어지는 문제를 프론트에서 보정하며 살았는데, 원인은 여기였을 가능성이 크다. → [[보드게임 - routes 분석 - 예약|routes 분석 - 예약]]

## 관련 노트

[[보드게임카페예약사이트 프로젝트 MOC]] · [[보드게임 - routes 분석 - 인증과 유저|routes 분석 - 인증과 유저]] · [[보드게임 - 전문용어 정리|전문용어 정리]]
