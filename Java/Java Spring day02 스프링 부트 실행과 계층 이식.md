...
스프링이 이름을 맞출 때 실제로 보는 것은 필드가 아니라 **메소드**다. `get`·`set` 을 떼고 남은 부분으로 이름을 만드는 자바빈 규약을 따르는데, 여기에 예외가 하나 붙어 있다. 뗀 뒤 **첫 글자를 소문자로 내리되, 앞의 두 글자가 모두 대문자면 그대로 둔다**는 조항이다. `URL`·`ID` 처럼 약어로 시작하는 이름이 `uRL` 로 뭉개지는 것을 막으려고 들어간 규칙이다.
...
등록 버튼에 걸어 둔 `onclick="boardSave()"` 는 같은 자리에 이어 붙일 다음 함수다. 조회 한 갈래가 뚫렸으니 나머지는 같은 모양에서 `axios.get` 을 `post`·`put`·`delete` 로 바꾸고, 보낼 값을 `.content`·`.writer` 입력칸에서 꺼내 실어 보내면 된다. 실제로 붙인 모양은 1-23에, `fetch` 로 같은 일을 하는 형태는 2-15에 적어 뒀다.
...
### 1-23. 등록과 수정 — 버튼이 실제로 서버를 부른다

1-22에서 조회 한 갈래를 뚫어 두고 `boardSave()` 는 이름만 걸어 둔 상태였다. 여기에 등록과 수정 두 함수를 채우면서 **화면에서 표를 바꾸는 길**이 열린다. 조회가 서버 → 화면 방향이었다면 이쪽은 화면 → 서버 방향이다.
...
async function boardSave() {
  ...
  const response = await axios.post(
    `/board/save?content=${content}&writer=${writer}`,
  );
  // 3. 결과 출력
...
세 단계가 뚜렷하다. **값 꺼내기 → 요청 보내기 → 결과 처리**다. 조회는 보낼 값이 없어서 두 번째 단계가 주소 하나로 끝났는데, 등록부터는 앞에 값을 모으는 자리가 붙는다.

**① `.value` — 입력칸에서 값을 꺼낸다**

`querySelector(".content")` 가 잡아 오는 것은 입력칸이라는 **마크업 요소**이고, 거기에 사람이 친 글자는 `.value` 속성에 들어 있다. [[JS day11 DOM 조작]] 에서 정리한 갈림이 그대로다.

| 표기 | 꺼내는 것 |
| --- | --- |
| `요소.value` | `<input>` 에 입력된 값 |
| `요소.innerHTML` | 그 요소 안의 마크업 문자열 |
| `요소.textContent` | 그 요소 안의 글자만 |

`<input>` 에서 값을 꺼낼 때는 `.value` 하나만 쓴다. 입력한 글자는 마크업 안쪽이 아니라 요소의 속성으로 들어가기 때문에, `innerHTML` 로는 잡히지 않는다.

**② 값을 주소 뒤에 붙여 보낸다**

```javascript
await axios.post(`/board/save?content=${content}&writer=${writer}`);
```

주소 뒤 `?` 부터가 쿼리 스트링이다. `이름=값` 을 `&` 로 이어 붙인 형태라, 서버 쪽 `save(BoardDto boardDto)` 가 이름을 맞춰 받는다(1-9 ⑤).

```
POST /board/save?content=안녕&writer=유재석
        │
        ├─ new BoardDto()
        ├─ setContent("안녕")     ← content 라는 이름을 찾아
        └─ setWriter("유재석")     ← writer 라는 이름을 찾아
```

POST인데 값이 주소에 실려 있는 것이 눈에 걸릴 수 있다. 스프링의 커맨드 객체 바인딩은 **주소의 쿼리 스트링과 요청 본문을 가리지 않고** 이름으로 값을 찾기 때문에 양쪽 다 닿는다. 다만 둘은 성질이 달라서, 어느 쪽에 실을지 정하는 기준은 2-19에 따로 정리했다.

**③ 상대주소로 적었다**

조회는 `http://127.0.0.1:8080/board/findall` 이라고 다 적었는데 여기서는 `/board/save` 만 적었다. 화면과 API가 같은 서버에서 나오니 둘 다 같은 곳에 닿는다. 상대주소로 적어 두면 포트를 바꾸거나 다른 곳에 올려도 코드를 고칠 일이 없다 — 2-18에 정리한 자리다.

**④ `response.data == true` — 돌려받은 값 판정**

서버의 `save()` 는 `boolean` 을 돌려주고(1-10 ④), `@RestController` 가 그것을 그대로 본문에 실어 보낸다(1-13). 브라우저에서는 JSON `true`·`false` 가 자바스크립트 `true`·`false` 로 담겨 `.data` 에 들어온다.

```
DAO의 boolean  ──▶  JSON true  ──▶  response.data (자바스크립트 boolean)
```

자바에서 시작한 값이 타입을 유지한 채 화면까지 온 셈이라, `if (response.data)` 로 바로 판정해도 된다. `== true` 를 붙이면 뜻이 눈에 더 잘 들어온다. 자바스크립트의 `==` 는 타입이 다르면 맞춰서 비교하고 `===` 는 타입까지 같아야 한다는 갈림이 있는데([[JS day03 자료형과 연산자]]), 값이 이미 `boolean` 이라 여기서는 결과가 같다.
...
등록이 끝나면 화면의 표는 아직 옛 목록이다. `boardFindAll()` 을 한 번 더 불러 **서버에서 다시 받아 그린다.** 화면을 통째로 새로 여는 `location.reload()`(2-15)와 비교하면 갈림이 분명하다.

| 방식 | 다시 받는 것 | 성질 |
| --- | --- | --- |
| `location.reload()` | HTML·JS·CSS 전부 | 간단하다. 화면이 깜빡이고 입력 상태가 사라진다 |
| `boardFindAll()` 재호출 | 목록 데이터만 | 바뀐 자리만 다시 그린다 |

함수를 나눠 둔 값어치가 여기서 나온다. 조회를 따로 함수로 만들어 뒀기 때문에 등록·수정·삭제 어디서든 끝에 한 줄만 붙이면 화면이 최신 상태가 된다.

`alert` 는 브라우저가 띄우는 알림창이다. 확인을 누를 때까지 다음 코드가 멈추므로 결과를 확인하며 만들기에 편한데, 화면 흐름을 끊는 성질이 있어 실제 서비스에서는 화면 안쪽에 메시지를 그려 넣는 쪽으로 간다.
...
| | 등록 | 수정 |
| --- | --- | --- |
| HTTP 방식 | `axios.post` | `axios.put` |
| 값을 얻는 곳 | 입력칸 `.value` | `prompt` 창 + 매개변수 `no` |
| 보내는 값 | `content`·`writer` | `no`·`content` |

**① `prompt` — 값 하나를 물어보는 창**

`prompt("수정할 내용: ")` 은 입력칸이 딸린 알림창을 띄우고, 사람이 친 글자를 문자열로 돌려준다. 취소를 누르면 `null` 이 온다는 점만 갈라 두면 된다.

| 함수 | 하는 일 | 돌려주는 것 |
| --- | --- | --- |
| `alert` | 알리기 | 없음 |
| `confirm` | 예·아니오 묻기 | `true` · `false` |
| `prompt` | 값 받기 | 입력한 문자열 · `null` |

셋 다 브라우저가 그리는 창이라 모양을 바꿀 수 없다. 값을 받는 자리를 화면 안에 두려면 등록처럼 `<input>` 을 만드는 쪽이고, 지금처럼 한 값만 잠깐 물을 때는 `prompt` 가 가볍다.

**② 매개변수로 대상을 받는다 — `boardUpdate(no)`**

등록은 매개변수가 없는데 수정에는 `no` 가 있다. **어느 줄을 고칠지**가 함수 밖에서 정해지기 때문이다. 표의 줄마다 수정 버튼이 하나씩 있고, 그 버튼이 자기 줄의 번호를 실어 함수를 불러 주는 구조다.

```
표의 3번 줄 수정 버튼 클릭  →  boardUpdate(3)  →  PUT /board/update?no=3&content=...
```

값을 함수에 넘기는 자리는 1-22에서 표를 그리는 반복문 안이다. 마크업을 문자열로 만들 때 `onclick` 에 번호를 같이 박아 두면 이어진다.

```javascript
html += `<tr>
  <td>${게시물객체.no}</td>
  ...
  <td><button onclick="boardUpdate(${게시물객체.no})">수정</button></td>
</tr>`;
```

`${ }` 안의 값이 문자열로 바뀌어 들어가므로, 3번 줄에는 `onclick="boardUpdate(3)"` 가 그대로 적힌 마크업이 만들어진다. 서버에서 받아 온 값이 **화면에 그려지는 동시에 다음 요청의 재료가 되는** 자리다.

**③ PUT 요청은 주소창으로 보낼 수 없다**

브라우저 주소창은 GET만 보낸다(2-11). `@PutMapping("/board/update")` 를 만들어 두고도 확인할 방법이 없었는데, `axios.put()` 을 쓰면서 처음으로 실제 PUT 요청이 나간다. 1-16에서 짝지어 둔 매핑 넷이 화면 코드와 하나씩 이어지는 중이다.

| 화면 함수 | axios | 서버 매핑 | 상태 |
| --- | --- | --- | --- |
| `boardFindAll()` | `.get` | `@GetMapping("/board/findall")` | 이어짐 |
| `boardSave()` | `.post` | `@PostMapping("/board/save")` | 이어짐 |
| `boardUpdate(no)` | `.put` | `@PutMapping("/board/update")` | 이어짐 |
| (삭제) | `.delete` | `@DeleteMapping("/board/delete")` | 남은 자리 |

**④ 서버의 `update` 는 내용만 바꾼다**

`update` SQL이 `set content = ? where no = ?` 였으므로(1-15) 보내는 값도 `no`·`content` 둘이다. 작성자까지 바꾸려면 SQL의 `set` 에 컬럼을 더하고 화면에서도 값을 하나 더 실어 보내야 한다. **화면이 보낼 수 있는 값의 범위를 SQL이 정하고 있다**는 점이 눈에 띄는 자리다.

#### 세 함수가 같은 모양을 하고 있다

```
값 얻기  →  await axios.방식(주소)  →  response.data 판정  →  boardFindAll()
```

등록·수정이 이 틀을 그대로 따르고, 남은 삭제도 같은 모양이 된다. 삭제는 물어볼 내용이 없으니 `prompt` 자리에 `confirm("삭제할까요?")` 를 두고, `axios.delete(\`/board/delete?no=${no}\`)` 를 부르면 셋과 같은 줄에 선다.
...
### 2-19. 값을 주소에 실을까, 본문에 실을까

1-23에서 등록·수정 둘 다 값을 주소 뒤 쿼리 스트링에 붙여 보냈다. 스프링은 양쪽을 가리지 않고 이름으로 찾아 채우므로 어느 쪽이든 닿는데, 성질이 갈리는 자리가 있어 정리해 둔다.

```javascript
// ① 주소에 싣기 — 쿼리 스트링
await axios.post(`/board/save?content=${content}&writer=${writer}`);

// ② 본문에 싣기 — 두 번째 인자로 객체를 넘긴다
await axios.post('/board/save', { content, writer });
```

`axios.post(주소, 객체)` 처럼 두 번째 인자를 주면 axios가 객체를 JSON으로 바꿔 **요청 본문**에 싣고 `Content-Type: application/json` 을 붙인다(1-9 ③).

| | 주소에 싣기 | 본문에 싣기 |
| --- | --- | --- |
| 어디에 남는가 | 브라우저 기록·서버 접근 로그에 주소째 남는다 | 본문이라 주소에는 안 남는다 |
| 길이 | 주소 길이 제한을 받는다 | 사실상 제약이 없다 |
| 눈으로 확인 | 주소만 봐도 값이 보인다 | 개발자 도구를 열어야 한다 |
| 특수문자 | `&`·`#`·공백이 들어가면 인코딩이 필요하다 | 그대로 실린다 |

**① 특수문자와 `encodeURIComponent`**

쿼리 스트링에서 `&` 는 값을 가르는 기호, `#` 는 문서 안 위치를 가리키는 기호다. 그래서 입력값에 이 글자가 섞이면 값이 거기서 끊긴 것처럼 읽힌다. 주소에 값을 실을 때 감싸 주는 함수가 있다.

```javascript
await axios.post(`/board/save?content=${encodeURIComponent(content)}`);
```

`encodeURIComponent` 는 주소에서 뜻을 가진 글자를 `%26` 같은 표기로 바꿔 준다. 본문으로 보내면 이 처리를 안 해도 되는 것이 두 번째 방식의 편한 점이다.

**② 서버가 받는 쪽도 갈린다**

값이 본문에 JSON으로 오면 커맨드 객체 바인딩(1-9 ⑤)만으로는 잡히지 않는다. JSON 본문을 객체로 받으려면 매개변수에 표시를 하나 붙인다.

| 보내는 형태 | 받는 쪽 |
| --- | --- |
| 쿼리 스트링 · 폼 인코딩 | `save(BoardDto dto)` — 표시 없이 이름으로 채워진다 |
| JSON 본문 | `save(@RequestBody BoardDto dto)` |

`@RequestBody` 는 본문 전체를 읽어 객체로 바꾸라는 표시다. 화면 쪽을 `axios.post(주소, 객체)` 로 바꾸면 서버 쪽에도 이 표시를 같이 달아야 짝이 맞는다.

**③ 어느 쪽을 쓸까**

값이 한둘이고 감춰야 할 것이 없으면 주소에 실어도 확인하기 편하다. 다만 **비밀번호·전화번호 같은 값은 주소에 남는다**는 성질이 그대로 따라오므로(2-13에서 자연키를 주소에 쓸 때 짚은 자리와 같다), 그런 값은 본문으로 보내는 편이 안전하다. 값이 여러 개거나 중첩 구조면 JSON 본문 쪽이 다루기도 낫다.
