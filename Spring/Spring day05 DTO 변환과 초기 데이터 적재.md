...
| `spring.sql.init.*` | 뜰 때 SQL 파일을 실행하기 (1-7) |
...
**JPA를 거치므로 감사 리스너가 돌아 시각이 자동으로 채워진다.** 1-8에서 `NOW()` 를 직접 적어야 했던 자리가 없어진다. 대신 SQL 파일보다 적는 양이 늘고, 이미 들어 있는지 확인하는 코드를 따로 둬야 한다.
